...
def load_class_mapping():
  ...
  if not os.path.exists(CLASS_MAPPING_FILE):
    sys.stderr.write(
      f'Warning: {CLASS_MAPPING_FILE} not found; no class categories available.\n'
    )
    return
  ...
  sys.stderr.write(
    f'Loaded {len(class_category_map)} class categories from {CLASS_MAPPING_FILE}.\n'
  )
...
def _append_new_class(cid: int, category: str):
  ...
  label = class_category_map.get(
    str(cid), ''
  )  # Unknown classes have no pre-existing label
  ...
  with open(input_filename, 'rb') as bz:
    ...
    try:
      for decompressed_line_bytes in stream_to_lbzip2(file_chunk_generator()):
        ...
        print(
          f'\rProcessed {current_bytes / (1024**3):.2f} GB of stream in '
          + f'{elapsed} s (avg {current_bytes / (1024**2) / elapsed:.2f} '
          + 'MB/s), wrote ('
...
def download_stream():
  ...
  try:
    response = requests.get(
      constants.WIKIDATA_ENTITIES_URL, headers=headers, stream=True, timeout=30
    )
  ...
  try:
    for decompressed_line_bytes in stream_to_lbzip2(
      ...
      try:
        ...
        print(
          f'Rolling back network stream to last known good checkpoint: {last_good_byte}'
        )
        response.close()
...
def extract_wikidata(line: bytes):
  if COORD_PROP_BYTES not in line and SUBCLASS_PROP_BYTES not in line:
    return
  line = line.strip()
  if line.endswith(b','):
    line = line[:-1]
  try:
    ...
    if 'claims' in entity and 'P625' in entity['claims']:
      ...
      coords = [
        ...
        for v in (
          entity['claims']['P625']
          if isinstance(entity['claims']['P625'], list)
          else [entity['claims']['P625']]
        )
      ]
      classes = (
        [
      ...
      if q_id and enwiki and coords:
        ...
        record['ct'] = codes
        with gzip.open(
          constants.WIKIDATA_COORDS_EXTRACT_FILE, 'at', encoding='utf-8'
        ) as earth_coords:
    ...
    if SUBCLASS_PROP_BYTES in line:
      with gzip.open(
        constants.WIKIDATA_SUBCLASS_EXTRACT_FILE, 'at', encoding='utf-8'
      ) as subclass:
