...
TAXONOMY_REPORT_FILE = (
  '/home/r/github/wp-map/process/taxonomy_analysis_report.json'
)
...
def analyze_pois():
  ...
  with gzip.open(POI_FILE, 'rb') as f:
    for line in f:
      ...
      if total_pois % 300000 == 0:
        print(
          f'    Processed {total_pois:,} POIs ({time.time() - start:.1f}s)...'
        )
  ...
  print(
    f'[+] POI Analysis complete in {elapsed:.1f}s across {total_pois:,} POIs!'
  )
  ...
  report = {
    ...
    'mapped_categories_distribution': dict(
      mapped_category_counts.most_common()
    ),
