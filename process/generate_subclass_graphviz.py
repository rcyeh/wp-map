...
def load_graph_and_poi_counts(min_poi_threshold=2500):
    ...
    for line in f:
      ...
      for c in classes:
        cid = (
          c
          if isinstance(c, int)
          else (c.get('numeric-id') if isinstance(c, dict) else None)
        )
        if cid:
          poi_counts[cid] += 1
  ...
  print(
    f'[+] Parsed {total_pois:,} POIs ({total_declarations:,} total subclass declarations).'
  )
  print(f'[+] Found {len(poi_counts):,} unique direct classes in POIs.')

  # Select top classes meeting threshold
  selected_class_ids = {
    cid for cid, count in poi_counts.items() if count >= min_poi_threshold
  }
  print(
    f'[*] Filtered {len(selected_class_ids):,} top classes with '
    f'>= {min_poi_threshold:,} POIs...'
  )
...
def generate_graphviz():
  min_threshold = 2500
  poi_counts, selected_ids, graph = load_graph_and_poi_counts(
    min_poi_threshold=min_threshold
  )
  ...
  print(
    f'[*] Building Graphviz DOT with {len(relevant_nodes):,} nodes and {len(edges):,} edges...'
  )

  dot_lines = []
  dot_lines.append('digraph POI_Subclass_Taxonomy {')
  dot_lines.append(
    '  graph [rankdir="BT", layout=dot, labelloc="t", '
    'label="Wikidata POI Subclass-Of Hierarchy (P279 / P31)\\nNodes '
    'labeled with [Class Name (QID)] and direct '
    'POI count", '
    'fontname="Helvetica", fontsize=16];'
  )
  dot_lines.append(
    '  node [shape=box, style="filled,rounded", '
    'fillcolor="#E8F0FE", color="#1A73E8", '
    'fontname="Helvetica", fontsize=10];'
  )
  dot_lines.append(
    '  edge [color="#5F6368", arrowhead="normal", fontname="Helvetica", fontsize=8];'
  )
  dot_lines.append('')
  ...
  dot_path = '/home/r/github/wp-map/process/poi_subclass_taxonomy.svg'
  os.system(f'dot -Tsvg {DOT_OUTPUT_FILE} -o {dot_path}')
  print(f'[+] Rendered SVG to {dot_path}')
