...
def write_javascript_tile_checker(tiles_written: dict[int, list]):
  tile_checker_path = os.path.join(
      constants.PATH_TO_TILE_DIR,
      constants.TILE_CHECKER_FILENAME
  )
  with open(tile_checker_path, "wt") as f:
    f.write('// Generated by process/write.py\n')
    f.write('export const tileSets = [\n')
    for z in range(MAXZOOM + 1):
      tile_list = tiles_written[z]
      f.write('  new Set([')
      f.write(f'{",".join(str((t.x << 15) | t.y) for t in tile_list)}')
      f.write(']),\n')
    f.write('];\n')
