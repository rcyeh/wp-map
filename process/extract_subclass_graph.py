...
Extracts the Wikidata subclass and instance-of graph directly into a
line-delimited `subclass_graph.jsonl.gz` format (one JSON object per line:
`{...}`).
"""
...
OUTPUT_SUMMARY_FILE = (
  '/home/r/github/wp-map/process/subclass_graph_summary.json'
)
...
def extract_graph(input_path=INPUT_SUBCLASS_FILE):
  ...
  print(
    f'[*] Extracting Wikidata subclass definitions line-by-line to {OUTPUT_GRAPH_FILE}...'
  )
  ...
  with (
    gzip.open(input_path, 'rb') as fin,
    gzip.open(OUTPUT_GRAPH_FILE, 'wb') as fout,
  ):
    for line in fin:
      total_lines += 1
      ...
      en_label = (
        labels.get('en', {}).get('value') if isinstance(labels, dict) else None
      )
